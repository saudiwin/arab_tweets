...
seq_val <- floor(length(old_days)/1)
new_days <- rep(1:seq_val,each=1)
if(length(new_days)<length(old_days)) {
  new_days <- c(new_days,rep(seq_val+1,times=length(old_days)-length(new_days)))
}
#coup_day_new <- new_days[which(old_days>coup_day & (old_days %% 1))[1]]
...
>>>>>>> 5fcb7501792873b2fb9c67daa116564df68a5c84
