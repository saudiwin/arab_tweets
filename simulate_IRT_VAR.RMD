...
We first do an $\hat{R}$ check to make sure the model converged.
...
ideal_pts <- as.data.frame(out_fit,pars='alpha_country') %>% 
  mutate(iter=1:n()) %>% 
...
ints_est <- as.matrix(out_fit,pars='alpha_int_full')
...
all_irfs1 <- bind_rows(list(`Islamists\nEgypt`=irf(adj_in=adj_in_est[,1:2],
  ...
  summarize(median_est=median(y_irf),
            upper=quantile(y_irf,.95),
            lower=quantile(y_irf,.05)) %>% 
