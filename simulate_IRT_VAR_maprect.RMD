---
title: "A Simulation of the ideal point influence model with parallel processing"
...
sigma2 <- c(.1,rnorm(sides,0.1,0.02))
country <- .5
```
...
missingd <- apply(all_data_array,2,function(c) sum(c[4:(length(gen_out)/cit)]==99999))
...
code_compile <- stan_model(file='irt_var_maprect_nonvarying_2d_v3.stan')
...
start_func <- function() {
  list(alpha=runif(t*sides,min = -0.5,max=0.5),
       ...
       alpha_int1=c(-1,0,0,0),
       alpha_int2=c(1,0,0,0),
       mean_delta=1,
       mean_beta=0,
       sigma_beta=1,
       sigma_delta=1,
...
out_fit <- sampling(code_compile,
                    ...
                    chains=4,
                    seed=6651)
saveRDS(list(model=out_fit,true=list(alpha=c(combine_vec1,combine_vec2),betax=c(betax1,betax2))),'data/out_fit_simulation_maprect_nonvarying_v3.rds')
```
