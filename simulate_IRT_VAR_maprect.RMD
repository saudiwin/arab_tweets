...
code_compile <- stan_model(file='irt_var_maprect_nonvarying_2d.stan')
...
out_fit <- sampling(code_compile,
                    ...
                    chains=2,
                    seed=6651)
saveRDS(list(model=out_fit,true=list(alpha=c(combine_vec1,combine_vec2),betax=c(betax1,betax2))),'out_fit_simulation_maprect.rds')
```
...
time_pts <- sapply(param_names,function(x) as.numeric(x[2]))
ideal_code <- sapply(param_names,function(x) as.numeric(x[3]))
