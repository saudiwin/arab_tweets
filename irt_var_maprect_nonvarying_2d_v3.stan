...
model {
  ...
  alpha_int2[2:4] ~ normal(0,1);
  
  alpha_int1[1] ~ normal(-1,.01);
  //alpha_int1[2] ~ normal(1,.01);
  alpha_int1[2:4] ~ normal(0,1);
  adj_out1 ~ normal(0,2);
  adj_in2 ~ normal(0,2);
  adj_out2 ~ normal(0,2);
  adj_in1 ~ normal(0,2);
  ...
  //sigma_time1 ~ inv_gamma(10,1); // constrain the variance to push for better identification
  //sigma_time2 ~ inv_gamma(10,1); // constrain the variance to push for better identification
  sigma_time1 ~ normal(0,3);
  sigma_time2 ~ normal(0,3);
  betax1 ~ normal(0,3);
  betax2 ~ normal(0,3);
