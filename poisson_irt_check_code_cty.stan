data {
  ...
  int<lower=1,upper=K> cc[N];  // student for observation n
  int<lower=0> y[N];   // outcome for observation n
  vector[4] start_vals;
}
parameters {    
  vector[K-1] delta_free_1d;                  // discriminations
  vector[K-1] delta_free_2d;
...
transformed parameters {
  vector[K] delta_1d;
  delta_1d = append_row(islamist,delta_free_1d);
}
