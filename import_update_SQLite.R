...
new_cairo <- dbConnect(SQLite(),'data/Cairo.sqlite')

# First load the new data into memory so we can modify it and put it in the original SQL file

all_new <- dbReadTable(new_cairo,'Cairo_tweets') %>% as_data_frame %>% 
...
ids <- dbGetQuery(current_users,'SELECT user_id,unique_id from users') %>% as_data_frame 
ids <- distinct(ids,unique_id,user_id)
all_new <- left_join(all_new,ids,by=c('user_id'='user_id'))
...
just_tweets <- select(all_new,created_at,status_id,text,source,in_reply_to_status_id,
                      ...
                      unique_id,retweet_status_id,coordinates) %>% 
  rename(tweet_id=status_id,posted_time=created_at,rr_tweet_id=retweet_status_id,
         platform=source,body=text,tweet_retweet_reply=in_reply_to_status_id,geo_cord=coordinates) %>% 
